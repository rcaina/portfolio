  ...
  try {
    switch (req.method) {
      case "PATCH":
        ...
        const specimens = await retoolPrisma.specimen.findMany({
          where: {
            id: {
              in: ids,
            },
        ...
        if (specimens.length !== ids.length) {
          return res
            .status(404)
            .json({ error: "One or more specimens not found" });
        }
        ...
        return res.status(200).json({ message: "Specimens activated" });
      default:
