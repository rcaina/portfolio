...
const updateServiceRequestRequisitionFileSchema = z.object({
  requisitionFormS3Key: z.string().min(1),
});

export type UpdateServiceRequestRequisitionFileParams = z.infer<
  typeof updateServiceRequestRequisitionFileSchema
>;

const updateServiceRequestRequisitionFile = async (
  serviceRequestPrisma: PrismaClient,
  orderId: string,
  data: UpdateServiceRequestRequisitionFileParams
) =>
  await serviceRequestPrisma.order.update({
    where: {
      id: orderId,
    },
    data: {
...
export type UpdateServiceRequestPatientResponse = Prisma.PromiseReturnType<
  typeof updateServiceRequestRequisitionFile
>;
...
export default async function handler(
  ...
  const parsedBody = updateServiceRequestRequisitionFileSchema.parse(req.body);
  ...
  if (req.method === "PUT") {
    try {
      const serviceOrder = await updateServiceRequestRequisitionFile(
