...
const updateServiceRequestTypeSchema = z.object({
  serviceTypeId: z.string().min(1),
});

export type UpdateServiceRequestTypeParams = z.infer<
  typeof updateServiceRequestTypeSchema
>;

const updateServiceRequestType = async (
  serviceRequestPrisma: PrismaClient,
  orderId: string,
  data: UpdateServiceRequestTypeParams
) =>
...
export type UpdateServiceRequestPatientResponse = Prisma.PromiseReturnType<
  typeof updateServiceRequestType
>;
...
export default async function handler(
  ...
  const parsedBody = updateServiceRequestTypeSchema.parse(req.body);
  ...
  const serviceType = await serviceRequestPrisma.serviceType.findUnique({
    where: {
      id: parsedBody.serviceTypeId,
    },
    select: {
      id: true,
    },
  ...
  if (req.method === "PUT") {
    try {
      const serviceOrder = await updateServiceRequestType(
