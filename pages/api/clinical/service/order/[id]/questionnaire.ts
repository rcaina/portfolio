...
const updateServiceRequestQuestionnaireSchema = z
  .object({
    // Add any specific properties you want to validate here
  })
  .catchall(z.any());

export type UpdateServiceRequestQuestionnaireParams = z.infer<
  typeof updateServiceRequestQuestionnaireSchema
>;

const updateServiceRequestQuestinaire = async (
  serviceRequestPrisma: PrismaClient,
  orderId: string,
  data: UpdateServiceRequestQuestionnaireParams
) =>
...
export type UpdateServiceRequestQuestinaireResponse = Prisma.PromiseReturnType<
  typeof updateServiceRequestQuestinaire
>;
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<
    UpdateServiceRequestQuestinaireResponse | { error: string }
  >
) {
  ...
  const parsedBody = updateServiceRequestQuestionnaireSchema.parse(req.body);
  ...
  if (req.method === "PUT") {
    try {
      const serviceRequest = await updateServiceRequestQuestinaire(
